package com.mhansda.newsapp.animation
...
)
{
    ...
    {
        val cardHeightPx = with(LocalDensity.current){(maxWidth - (padding*2)).toPx()}
        val cardWidthPx = with(LocalDensity.current){(imageHeight - padding).toPx()}
